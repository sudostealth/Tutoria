...
interface StatHotspotsMapProps {
  language: Language;
  stats: SiteStats | null;
}

export const StatHotspotsMap: React.FC<StatHotspotsMapProps> = ({
  language,
  stats
}) => {
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-base font-extrabold text-white tracking-tight">
                {isBn ? 'বাংলাদেশের টিউশন হটস্পট ম্যাপ (লাইফটাইম)' : 'Bangladesh Tuition Hotspots (Lifetime)'}
              </h3>
            ...
            <p className="text-xs text-slate-300 font-normal">
              {isBn
                ? `ডাটাবেজে রেকর্ডকৃত মোট ${totalActivePosts} টি টিউশন পোস্ট`
                : `Accurately fetched from database: ${totalActivePosts} lifetime tuition requests`}
            </p>
          </div>
        </div>
      </div>
